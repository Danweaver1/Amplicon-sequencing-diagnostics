...
for (i in seq_along(file_list)) {
  ...
  MSC$Sample <- MSC$Sample %>% str_replace('LOD[0-9]', '') %>% str_replace('R', '') %>% str_replace("MiSeq","") %>% str_replace("\\.1_","LOD1 100 copies ") %>% str_replace("\\.2_","LOD2 50 copies ") %>% str_replace("\\.3_","LOD3 20 copies ") %>% str_replace("\\.4_","LOD4 10 copies ") %>% str_replace("\\.5_","LOD5 2 copies ") 
  tiff(plot_name,width = 875, height = 600)
  print(ggplot(MSC, aes(x=Sample,y=Proportional_counts, fill=Species)) +
...
for (i in seq_along(file_list)) {
  ...
  MSC$Sample <- MSC$Sample %>% str_replace('LOD[0-9]', '') %>% str_replace('R', '') %>% str_replace("MiSeq","") %>% str_replace("\\.1_","LOD1 100 copies ") %>% str_replace("\\.2_","LOD2 50 copies ") %>% str_replace("\\.3_","LOD3 20 copies ") %>% str_replace("\\.4_","LOD4 10 copies ") %>% str_replace("\\.5_","LOD5 2 copies ") 
  tiff(plot_name,width = 875, height = 600)
  print(ggplot(MSC, aes(x=str_replace(Sample,"[1-5]_",""),y=Counts, fill=Species)) +
          #set colours using colour table, set labels using formatted species names 
