...
df[,"PE_ITS_ID"] <- df[,"PE_ITS_iSeq_ID"] + df[,"PE_ITS_iSeqrep2_ID"]  + df[,"PE_ITS_MiSeq_ID"]
df[,"PE_ITS_Missing"] <- df[,"PE_ITS_iSeq_Missing"] +  df[,"PE_ITS_iSeqrep2_Missing"] + df[,"PE_ITS_MiSeq_Missing"]
#recalculate %ID

df[,"PE_ITS_ID_rate"] <- (df[,"PE_ITS_ID"]/(df[,"PE_ITS_ID"] + df[,"PE_ITS_Missing"]))*100
df[,"PE_ITS_ID_rate"]


#reduce and add + to indicate human background to colnames
library(stringr)
colnames(df) <- colnames(df) %>% str_replace_all("[_]", " ") %>% str_replace("PE ","")  %>% str_replace("merge ","") %>% str_replace("ID rate","+") %>% str_replace("IDrate","+") %>% str_replace("ITS","ITS1")
...
df1[,"PE_ITS_ID"] <- df1[,"PE_ITS_iSeq_ID"] + df1[,"PE_ITS_iSeqrep2_ID"]  + df1[,"PE_ITS_MiSeq_ID"]
df1[,"PE_ITS_Missing"] <- df1[,"PE_ITS_iSeq_Missing"] +  df1[,"PE_ITS_iSeqrep2_Missing"] + df1[,"PE_ITS_MiSeq_Missing"]
#recalculate %ID
df1[,"PE_ITS_ID_rate"] <- (df1[,"PE_ITS_ID"]/(df1[,"PE_ITS_ID"] + df1[,"PE_ITS_Missing"]))*100

colnames(df1) <- colnames(df1) %>% str_replace_all("[_]", " ") %>% str_replace("PE ","")  %>% str_replace("merge ","") %>% str_replace("ID rate","-") %>% str_replace("IDrate","-") %>% str_replace("ITS","ITS1")


full_tbl <- cbind(df[,11:12],df1[,11:12])
...
#improve plot text and plot only PE ITS 
