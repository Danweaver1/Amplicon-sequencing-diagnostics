## Performs chi squared tests on expected vs. experimental results for iSeq data
...
for (i in seq_along(file_list)) {
  ...
  TEF_iSeq=paste("TEF_Exp_sp_",samplename, "_iSeq_cutoff_filtered_0_2_prop_adjusted.csv", sep="")
  ITS_iSeq=paste("ITS_Exp_sp_",samplename, "_iSeq_cutoff_filtered_0_2_prop_adjusted.csv", sep="")
  ITS_iSeqrep2=paste("ITS_Exp_sp_",samplename, "_iSeqrep2_cutoff_filtered_0_2_prop_adjusted.csv", sep="")
  #check they both exist (rep1)
  if (file.exists(TEF_iSeq)) {
    #read in files
    TEFsample_counts <- read.table(TEF_iSeq,header=TRUE,sep=",")
  ...
  if (file.exists(ITS_iSeq)) {
    #read in files
    ITSsample_counts <- read.table(ITS_iSeq,header=TRUE,sep=",")
  ...
  if (file.exists(ITS_iSeqrep2)) {
    #read in files
    ITSrep2sample_counts <- read.table(ITS_iSeqrep2,header=TRUE,sep=",")
    if (sum(ITSrep2sample_counts[,4] != 0)) {
      
      ###ITS chi with sample proportional counts
      ITSrep2_res_pro <- chisq.test(ITSrep2sample_counts[,4],p=mock_counts[,4])
      
      pvals<- c(ITSrep2_res_pro$p.value)
      pval_table <- as.data.frame(pvals,row.names=c("ITSrep2pro"))
      pval_table[2] <- rep(samplename,1)
      colnames(pval_table)<- c("pvals","sample")
      write.csv(pval_table,paste("ITSrep2_", samplename,"_pvals.csv",sep=""), row.names=T) 
    } else {
      print(paste("Empty count files found for ITSrep2",samplename,sep=" "))
    } 
  } else {
    print(paste("All sample files not available for ITSrep2",samplename,sep=" "))
  }
}
